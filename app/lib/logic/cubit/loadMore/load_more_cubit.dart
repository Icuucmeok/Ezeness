...
class LoadMoreCubit extends Cubit<LoadMoreState> {
  ...
  static const String loadMoreExploreSectionFunction = "loadMoreExploreSectionPost";
  ...
  void loadMoreExploreSectionPost(PaginationPage page,
      {required String sectionKey,required String tabType,bool isFromListViewScreen = false}) async {
    emit(LoadMoreLoading());
    try {
      final data = await _postRepository.getExploreSectionPostList(sectionKey,tabType, page: page);
      if (isFromListViewScreen) {
        emit(LoadMorePostListViewScreenLoaded(data!));
      } else {
  ...
  void getExploreTabPostList(PaginationPage page, {required String tabType}) async {
    emit(LoadMoreLoading());
    try {
      final data = await _postRepository.getExploreTabPostList(tabType, page: page);
  ...
  void loadMoreMyPost(PaginationPage page,
      {bool isFromListViewScreen = false,
      String? search}) async {
    emit(LoadMoreLoading());
    try {
      final data = await _postRepository.getMyPosts(
  ...
  void loadMorePostViewScreen(PaginationPage page,
    ...
    try {
      final data =
          await _postRepository.getPosts(postType:postType, isKids: isKids, page: page,randomCode: randomCode);
  ...
  void loadMorePostListViewScreen(
    ...
    if (body.loadMoreFunctionName == loadMoreExploreSectionFunction) {
      loadMoreExploreSectionPost(isFromListViewScreen: true,sectionKey:body.sectionKey!,tabType: body.tabType!,page);
    }
  }
  ...
  void loadMoreSearchTagUpPost(PaginationPage page, {int isKids=0,String? search,bool withIsKids=true,int withLiftUp=1}) async {
    emit(LoadMoreLoading());
    try {
      final data =  await _postRepository.getPosts(page: page,isKids: isKids,search: search,withIsKids:withIsKids,withLiftUp: withLiftUp);
  ...
  void loadMoreSearchTagUpProfile(PaginationPage page, {String search=''}) async {
    emit(LoadMoreLoading());
    try {
      final data =  await _profileRepository.getUsers(page: page,search: search);
