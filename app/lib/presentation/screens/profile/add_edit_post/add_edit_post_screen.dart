...
class _AddEditPostScreenState extends State<AddEditPostScreen> {
          ...
          Container(
            ...
            child: Center(
              child: Card(
                ...
                child: BlocBuilder<AddEditPostCubit, AddEditPostState>(
                  buildWhen: (previous, current) => previous != current,
                  builder: (context, state) {
                    if (state is AddEditPostProgress) {
                      return Padding(
  ...
  Future<void> addEditPostFunction() async {
    ...
    for (int i = 0; i < mediaList.length; i++) {
      ...
      File? compressedFile;
      int fileSizeInMB = originalFile.lengthSync() ~/ (1024 * 1024);

      print("original file size in MB: $fileSizeInMB");
      if (Constants.imageExtantions.contains(fileExtension)
          // &&
          // fileSizeInMB > 70
          ) {
        compressedFile = await compressImage(originalFile);
        int compressed = compressedFile!.lengthSync() ~/ (1024 * 1024);
        print("compress image file size in MB: ${compressed}");
      } else if (Constants.videoExtentions.contains(fileExtension)) {
        compressedFile = await compressVideo(originalFile);
        int compressed = compressedFile.lengthSync() ~/ (1024 * 1024);
        print("compress video file size in MB: ${compressed}");
      } else {
