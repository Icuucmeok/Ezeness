      ...
      if (requestType == HttpRequestType.get) {
        ...
        if (postRequestType == PostRequestType.none) {
          ...
          for (AppFile file in files) {
            ...
            // Uncomment if video compression is needed
            // if (Helpers.isVideoExtension(file.fileExtension!)) {
            //   if (File(file.filePath!).lengthSync() > 10000000) {
            ...
            //     final output = await Compressor.compressVideo(file.filePath!, compressedPath);
            //     request.files.add(await http.MultipartFile.fromPath(
            //       file.fileKey!,
            //       output!,
            //       filename: file.fileName,
          ...
          int totalBytes = streamResponse.contentLength ?? 0;
          int uploadedBytes = 0;
          ...
          streamResponse.stream.listen((chunk) {
            ...
            if (totalBytes > 0 && onProgress != null) {
              onProgress(
                  (uploadedBytes / totalBytes) * 100); // Progress in percentage
            }
          });
