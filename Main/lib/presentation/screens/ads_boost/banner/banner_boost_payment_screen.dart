...
class BannerBoostPaymentScreen extends StatefulWidget {
  static const String routName = 'banner_payment';

  const BannerBoostPaymentScreen({
    Key? key,
  ...
  @override
  State<BannerBoostPaymentScreen> createState() =>
      _BannerBoostPaymentScreenState();
}

class _BannerBoostPaymentScreenState extends State<BannerBoostPaymentScreen> {
  late User user;
  late AddBannerBoostCubit _addBannerBoostCubit;
  ...
  void initState() {
    ...
    _addBannerBoostCubit = context.read<AddBannerBoostCubit>();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text(S.current.bannerBoostPayment),
          elevation: 0,
        ),
        body: ListView(
          padding:
          ...
          children: [
            ...
            BlocConsumer<AddBannerBoostCubit, AddBannerBoostState>(
              listener: (context, state) async {
                if (state is AddBannerBoostDone) {
                  await _profileCubit
                ...
                if (state is AddBannerBoostFailure) {
              ...
              builder: (context, state) {
                return CustomElevatedButton(
                  isLoading: state is AddBannerBoostLoading,
                  ...
                  onPressed: user.wallet == null ||
                          (num.tryParse(user.wallet!) ?? 0) <= 0
                      ? null
                      : () => _addBannerBoostCubit.addBannerBoost(
                            file: widget.bannerFile,
                            planId: widget.plan.id,
                            postId: widget.post.id,
                            startDate: widget.plan.availableDate, // TODO:
                          ),
                  child: Text(
                    S.current.pay,
                    style: Theme.of(context).textTheme.displayLarge?.copyWith(
